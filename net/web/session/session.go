...
type Store interface {
	...
	Destory(*web.Context) error
	// RegenerateId regenerates a session store from old session ID to new one.
	RegenerateId(*web.Context) (RawStore, error)
...
func Sessioner(options ...Options) web.Handler {
	...
	return func(ctx *web.Context) {
		...
		// Get flash.
		vals, _ := url.ParseQuery(ctx.GetCookie("web_flash"))
		if len(vals) > 0 {
			f := &Flash{Values: vals}
			...
			ctx.Data["Flash"] = f
			ctx.SetCookie("web_flash", "", -1, opt.CookiePath)
		}

		f := &Flash{ctx, url.Values{}, "", "", "", ""}
		ctx.Resp.Before(func(web.ResponseWriter) {
			if flash := f.Encode(); len(flash) > 0 {
				ctx.SetCookie("web_flash", flash, 0, opt.CookiePath)
			}
		})
...
func (m *Manager) Start(ctx *web.Context) (RawStore, error) {
...
func (m *Manager) Destory(ctx *web.Context) error {
	sid := ctx.GetCookie(m.opt.CookieName)
	if len(sid) == 0 {
		return nil
	}
...
// RegenerateId regenerates a session store from old session ID to new one.
func (m *Manager) RegenerateId(ctx *web.Context) (sess RawStore, err error) {
	sid := m.sessionId()
